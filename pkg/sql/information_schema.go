...
var informationSchema = virtualSchema{
	...
	tableDefs: map[descpb.ID]virtualSchemaDef{
		...
		catconstants.InformationSchemaNodeStmtStatsTableID:                informationSchemaNodeStmtStatsTable,
	},
	...
	populate: func(ctx context.Context, p *planner, dbContext catalog.DatabaseDescriptor, addRow func(...tree.Datum) error) error {
		...
		return forEachTableDesc(ctx, p, dbContext, opts, func(
			...
			for _, fk := range table.OutboundForeignKeys() {
				...
				refSchema, err := p.Descriptors().ByIDWithLeased(p.Txn()).Get().Schema(ctx, refTable.GetParentSchemaID())
				if err != nil {
					return err
				}
				if err := addRow(
...
var informationSchemaNodeStmtStatsTable = virtualSchemaTable{
	...
	schema: `
CREATE TABLE information_schema.crdb_node_statement_statistics (
