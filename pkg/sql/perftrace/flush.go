...
func (f *Flusher) flush(ctx context.Context) error {
	...
	// Step 2: Drain the sampler
	spans := f.collector.Sampler().Drain()
	if len(spans) == 0 {
		return nil
	}
...
func (f *Flusher) insertSpans(ctx context.Context, spans []WorkSpan) error {
	...
	// INSERT INTO system.work_span (id, parent_id, node_id, statement_fingerprint_id, ts, duration, cpu_time, contention_time, component)
...
// serializeMapToJSON serializes a map to a JSON string, returning nil if the map is empty.
// Returns a string (not []byte) so that the SQL driver correctly interprets it as JSONB.
func serializeMapToJSON[V any](m map[string]V) interface{} {
	if len(m) == 0 {
		return nil
	}
...
// serializeQueryTagsToJSON serializes query tags to a JSON string.
// Returns a string (not []byte) so that the SQL driver correctly interprets it as JSONB.
func serializeQueryTagsToJSON(tags []QueryTag) interface{} {
	if len(tags) == 0 {
		return nil
	}
...
func (f *Flusher) insertBatch(ctx context.Context, spans []WorkSpan) error {
	...
	for _, span := range spans {
		...
		args = append(args,
			...
			span.Component,
			serializeMapToJSON(span.ComponentMetrics),
			serializeMapToJSON(span.ComponentAttributes),
			serializeQueryTagsToJSON(span.QueryTags),
		)
		argIdx += numColumns
	}
	...
	query := fmt.Sprintf(`
		...
		(id, parent_id, node_id, statement_fingerprint_id, ts, duration, cpu_time, component, component_metrics, component_attributes, query_tags)
		VALUES %s
