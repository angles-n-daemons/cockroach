...
// sql.count:n3 - 1
//
// In some use cases however, having a value for every node is more
// confusing than useful. Its for these reasons that we are adding the
// cluster metrics. In this layout, instead of each node keeping track of
// its own value for a metric, the metric has a single, cluster wide value.
// Using the same example from above, let's say we added a
// "cluster.sql.count" metric, a cluster metric equivalent of the above
// "sql.count" metric. When the system is scraped and the data stored in the
// time series database only a single value for the entire cluster is ever
// stored, eg:
//
// cluster.sql.count - 9
//
// The value of this metric, instead of being tracked by each node in
// memory, will be kept in a system table, system.cluster_metrics, which can
// be thought of as a collection of eventually consistent cluster
...
// util/metric.Metrics to its own managed registry. This registry is accessible
...
// registered util/metric.Metric to accumulate updates, and then flushes
