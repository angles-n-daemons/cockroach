...
func infoByImpl() map[string]implInfo {
	...
	return map[string]implInfo{
		...
		"Prometheus Native":   {promNativeBucketCount * 8, fmt.Sprintf("%d", promNativeBucketCount)},
...
func writeSpeedReport(out *strings.Builder, sd speedData) {
	...
	out.WriteString("(run benchmarks for authoritative numbers; these are inline single-sample)\n\n")
