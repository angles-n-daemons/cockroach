...
func TestPrometheusExport(t *testing.T) {
	...
	t.Run("native keys align with prometheus schema", func(t *testing.T) {
		...
		// Build a set of expected keys from promBucketKey.
...
func TestWindowedHistogramBasic(t *testing.T) {
	...
	// Windowed snapshot should also have all values (first scrape).
	winSnap := wh.WindowedSnapshot()
	require.Equal(t, uint64(100), winSnap.TotalCount)

	// After scrape, the window should be empty.
	winSnap2 := wh.WindowedSnapshot()
	require.Equal(t, uint64(0), winSnap2.TotalCount)
...
func TestWindowedHistogramMultipleScrapes(t *testing.T) {
	wh := NewWindowed(1, 1000, 0.05)

	// Scrape 1: record 50 values.
	for i := int64(1); i <= 50; i++ {
		wh.Record(i)
	}
	snap1 := wh.WindowedSnapshot()
	require.Equal(t, uint64(50), snap1.TotalCount)

	// Scrape 2: record 30 more values.
	for i := int64(51); i <= 80; i++ {
		wh.Record(i)
	}
	snap2 := wh.WindowedSnapshot()
	require.Equal(t, uint64(30), snap2.TotalCount)

	// Scrape 3: record 20 more.
	for i := int64(81); i <= 100; i++ {
		wh.Record(i)
	}
	snap3 := wh.WindowedSnapshot()
	require.Equal(t, uint64(20), snap3.TotalCount)
...
func TestWindowedHistogramConcurrent(t *testing.T) {
	...
	// Windowed snapshot should also have all (no scrape happened yet).
	winSnap := wh.WindowedSnapshot()
	require.Equal(t, uint64(goroutines*recordsPerGoroutine), winSnap.TotalCount)

	// After scrape, window is empty.
	winSnap2 := wh.WindowedSnapshot()
	require.Equal(t, uint64(0), winSnap2.TotalCount)
}
