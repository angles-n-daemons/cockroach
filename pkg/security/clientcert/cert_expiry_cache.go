...
func (c *ClientCertExpirationCache) MaybeUpsert(
	...
	certs[serial] = certInfo{
		...
		last_seen:  time.Unix(c.timeNow(), 0),
	}
}
...
func (c *ClientCertExpirationCache) Clear() {
	c.mu.Lock()
	defer c.mu.Unlock()
	for user := range c.mu.cache {
		for serial := range c.mu.cache[user] {
...
func (c *ClientCertExpirationCache) Len() int {
	...
	defer c.mu.Unlock()
	count := 0
	c.mu.expirationMetrics.Each(nil, func(_ *io_prometheus_client.Metric) {
		count++
	})
	return count
}
